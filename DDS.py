...
def runStacking():
    global stacking_acc
    text.delete('1.0', END)
    estimators = [('rf', RandomForestClassifier(n_estimators=10, random_state=42)),('ML', make_pipeline(StandardScaler(),MLPClassifier(random_state=1, max_iter=10)))]
    cls = StackingClassifier(estimators=estimators, final_estimator=RandomForestClassifier(n_estimators=10, random_state=42))
    cls.fit(X_train, y_train)
    text.insert(END,"Stacking Classifier Prediction Results\n") 
    prediction_data = prediction(X_test, cls) 
    stacking_precision = precision_score(y_test, prediction_data,average='macro') * 100
    stacking_recall = recall_score(y_test, prediction_data,average='macro') * 100
    stacking_fmeasure = f1_score(y_test, prediction_data,average='macro') * 100
    stacking_acc = accuracy_score(y_test,prediction_data)*100
    text.insert(END,"stacking Perceptron Precision : "+str(stacking_precision)+"\n")
    text.insert(END,"stacking Perceptron Recall : "+str(stacking_recall)+"\n")
    text.insert(END,"stacking Perceptron FMeasure : "+str(stacking_fmeasure)+"\n")
    text.insert(END,"stacking Perceptron Accuracy : "+str(stacking_acc)+"\n")

def runDDS_stacking():
    global classifier
    global dds_acc
    # Define base models
    base_models = [('rf', RandomForestClassifier(n_estimators=45, random_state=42)),
                   ('mlp', MLPClassifier(hidden_layer_sizes=(128, 64), activation='relu', solver='adam', max_iter=500))]
    # Define meta model
    meta_model = LogisticRegression()
    # Define stacking classifier
    selector = StackingClassifier(estimators=base_models, final_estimator=meta_model, cv=5)
    # Fit stacking classifier on training data
    selector = selector.fit(X_train, y_train)
    text.insert(END, "DDS Prediction Results\n") 
    # Make predictions on test data
    prediction_data = selector.predict(X_test) 
...
def graph():
    height = [rf_acc, mlp_acc,stacking_acc]
    bars = ('Random Forest Accuracy','MLP Accuracy','Stacking Accuracy')
...
font = ('times', 18, 'bold')
title = Label(main, text='A Driving Decision Strategy(DDS) Based on stacking approach')
...
ddsButton = Button(main, text="Run DDS with Stacking Algorithm", command=runStacking)
ddsButton.place(x=380,y=600)
ddsButton.config(font=font1)

graphButton = Button(main, text="Accuracy Stacking", command=graph)
